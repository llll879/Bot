...
async def cmd_start(message: types.Message):
    add_user(message.from_user.id) # Регистрация
...
async def cmd_exit(message: types.Message):
    add_user(message.from_user.id) # Регистрация (на всякий случай)
...
async def set_mode(message: types.Message):
    add_user(message.from_user.id) # АВТО-РЕГИСТРАЦИЯ ПРИ ВЫБОРЕ РЕЖИМА
    mode = "support" if "поддержка" in message.text.lower() else "chat"
    user_modes[message.from_user.id] = mode
    await message.answer(
        f"✅ Режим '{mode}' активен. Пишите сообщение.",
...
async def handle_user_msg(message: types.Message):
    add_user(message.from_user.id) # АВТО-РЕГИСТРАЦИЯ ПРИ ЛЮБОМ СООБЩЕНИИ
...
async def main():
    init_db()
    print("Бот запущен с авто-регистрацией всех пользователей!")
